...
def _has_conflict(db: Session, room_id: int, start: datetime, end: datetime) -> bool:
    existing = db.query(Booking).filter(Booking.room_id == room_id, Booking.status == "confirmed").all()
    for b in existing:
...
def _check_quota(db: Session, user_id: int, now: datetime, start: datetime) -> None:
    ...
    count = (
        db.query(Booking)
        .filter(Booking.user_id == user_id, Booking.status == "confirmed", Booking.start_time > now, Booking.start_time <= window_end)
        .count()
    )
...
@router.get("/bookings")
def list_bookings(page: int = Query(1, ge=1), limit: int = Query(10, ge=1, le=100), db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
def get_booking(booking_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    booking = db.query(Booking).join(Room, Booking.room_id == Room.id).filter(Booking.id == booking_id, Room.org_id == user.org_id).first()
    if booking is None:
    ...
    response["refunds"] = [{"amount_cents": r.amount_cents, "status": r.status, "processed_at": iso_utc(r.processed_at)} for r in booking.refunds]
    return response
...
def cancel_booking(booking_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    with _booking_lock:
        ...
        result = {"id": booking.id, "status": "cancelled", "refund_percent": refund_percent, "refund_amount_cents": entry.amount_cents}
